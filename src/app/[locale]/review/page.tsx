...
export default function ReviewPage() {
  ...
  function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith('image/')) {
      setErrors(p => ({ ...p, logo: t('errImage') }))
      return
    }
    if (file.size > 50 * 1024 * 1024) {
      setErrors(p => ({ ...p, logo: t('errTooLarge') }))
      return
    }
    setErrors(p => ({ ...p, logo: '' }))
    setLogoFile(file)
  ...
  function validate() {
    ...
    if (!form.business_name.trim()) e.business_name = t('errRequired')
    if (!form.full_name.trim()) e.full_name = t('errRequired')
    if (!form.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) e.email = t('errEmail')
    if (!form.about.trim()) e.about = t('errAbout')
    return e
  }
        ...
        <div className="max-w-lg mx-auto">
          {submitted ? (
            <div className="card p-12 text-center">
              ...
              <h2 className="font-display text-2xl font-extrabold text-slate-800 mb-3">{t('successTitle')}</h2>
              <p className="text-slate-500 leading-relaxed">
                {t.rich('successText', {
                  name: form.full_name,
                  email: form.email,
                  strong: (chunks) => <strong className="text-slate-700">{chunks}</strong>,
                })}
              </p>
            </div>
          ) : (
            <div className="card p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {/* Logo upload */}
                <Field error={errors.logo} label={t('logoLabel')}>
                  {logoPreview ? (
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold text-slate-600">{t('logoUpload')}</span>
                      <span className="text-xs text-slate-400">{t('logoHint')}</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field error={errors.full_name} label={t('nameLabel')} req>
                    <input className={`form-input ${errors.full_name ? 'form-input-error' : ''}`} placeholder={t('namePlaceholder')} {...f('full_name')} />
                  </Field>
                  <Field error={errors.email} label={t('emailLabel')} req>
                    <input type="email" className={`form-input ${errors.email ? 'form-input-error' : ''}`} placeholder={t('emailPlaceholder')} {...f('email')} />
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <Field error={errors.business_name} label={t('businessLabel')} req>
                    <input className={`form-input ${errors.business_name ? 'form-input-error' : ''}`} placeholder={t('businessPlaceholder')} {...f('business_name')} />
                  </Field>
                  <Field label={t('phoneLabel')}>
                    <input type="tel" className="form-input" placeholder={t('phonePlaceholder')} {...f('phone')} />
                  </Field>
                </div>
                <Field label={t('websiteLabel')}>
                  <input className="form-input" placeholder={t('websitePlaceholder')} {...f('website')} />
                </Field>
                <Field error={errors.about} label={t('aboutLabel')} req>
                  <textarea
