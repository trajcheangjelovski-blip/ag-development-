...
const MKD_FALLBACK: Record<string, number> = { basic: 3000, team: 5000, office: 18000 }
...
function StepBar({ step, mobile }: { step: number; mobile: boolean }) {
  const t = useTranslations('orderIt')
...
function ITOrderContent() {
  ...
  // Live prices from the admin panel (catalog id = `it-${card id}`). Before they
  // load, MK falls back to denar prices so the price doesn't flash USD first.
        ...
        {step === 1 && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '28px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#2563eb', margin: '0 0 10px' }}>{t('step1Eyebrow')}</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '28px' }}>
              {IT_PLANS_LIVE.map(p => {
                ...
                return (
                  ...
                  >
                    {p.popular && (
                      <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: '#2563eb', color: 'white', fontSize: '10px', fontWeight: 700, padding: '3px 12px', borderRadius: '100px', whiteSpace: 'nowrap' }}>{t('mostPopular')}</div>
                    )}
                    {sel && (
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit}>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#2563eb', margin: '0 0 10px' }}>{t('step2Eyebrow')}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 300px' : '1fr', gap: '24px', alignItems: 'start' }}>
              ...
              <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #e2e8f0', padding: mobile ? '20px 16px' : tablet ? '24px 20px' : '32px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    <label style={label}>{t('businessName')} <span style={{ color: '#ef4444' }}>*</span></label>
                  ...
                  <div>
                    <label style={label}>{t('fullName')} <span style={{ color: '#ef4444' }}>*</span></label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    <label style={label}>{t('email')} <span style={{ color: '#ef4444' }}>*</span></label>
                ...
                <div style={{ marginBottom: '16px' }}>
                  <label style={label}>{t('teamSize')}</label>
                  <input style={inputStyle} placeholder={t('teamSizePlaceholder')} value={form.teamSize} onChange={e => setForm(f => ({ ...f, teamSize: e.target.value }))} />
                </div>

                <div style={{ marginBottom: '16px' }}>
                  <label style={label}>{t('message')}</label>
                  <textarea rows={4} style={{ ...inputStyle, resize: 'vertical', minHeight: '100px' }} placeholder={t('messagePlaceholder')} value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} />
                </div>

                <div style={{ marginBottom: apiError ? '16px' : 0 }}>
                  <label style={label}>{t('coupon')}</label>
